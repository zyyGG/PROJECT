    ...
    actions:{
        ...
        addTask(context,value){
            if(value){
                ...
                if(newDatas.position>context.state.localPage*context.state.pagination) newDatas.isShow=false;
        ...
        ChangePage(context,value){
            // 如果翻页没有超出上限 那就可以增加
            if(context.state.localPage+value<=Math.ceil(context.state.listDatas.length/context.state.pagination)&&context.state.localPage+value>0)
            {
